...
class BuscaJobV2SupabaseUser extends BaseAuthUser {
  BuscaJobV2SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> buscaJobV2SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BuscaJobV2SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
